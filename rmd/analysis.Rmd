...
lollipop_chart <- function(full_data, item, title, xlab, nudge_x = 0, percent = TRUE, fun = ~mean(., na.rm = TRUE)) {
  summary_by_q <-
    full_data %>%
  ...
  ggplot(summary_all) +
    ...
    geom_text(data = summary_overall,
              aes(!!sym(item), question, label = if(percent) scales::percent(!!sym(item), 1) else round(!!sym(item), 2)), color = "black", 
              hjust = 0, nudge_x = nudge_x) +
    labs(x = xlab, y = "") +
    scale_color_manual(values = c("lightgreen","lightblue")) +
    {if (percent) scale_x_continuous(labels = scales::percent)} +
    ggtitle(title) +
    theme(legend.position = "none")
}
  
quantile_chart <- function(full_data, item, title, xlab, ylab, qlab, meanlab, xmax, nudge_x){
  plot_data <- full_data %>%
    group_by_at(item) %>%
  ...
  mean_data <- tibble(
    ...
    lab = sprintf(meanlab, round(x_mean))
  )
...
plots[[item]] <- bar_chart(
  full_data, item,
  item, "number of clicks", "% of users", 2)
...
plots[[item]] <- quantile_chart(
  full_data, item,
  "Time to complete a search task",
...
plots[[item2]] <- lollipop_chart(
  full_data, item, 
  "Average time to complete search task", "time (seconds)",
  fun = ~ mean(., na.rm =TRUE),
  percent = FALSE,
  nudge_x = 8)
...
plots[[item]] <- quantile_chart(
  ...
  "time (seconds)", "% of users who clicked at least this amount",
  meanlab = "Average : %s times", qlab = "%s%% click at least %s times",
  xmax = 30, nudge_x = 1
)
