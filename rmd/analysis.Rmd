...
quantile_chart2 <- function(full_data, item, title, xlab, ylab, qlab, meanlab, xmax, 
  ...
  plot_data %>%
    ...
    ggtitle(title) +
    labs(x = xlab, y = ylab) +
    geom_point(aes(x_q1,y_q1), size = 4) +
    geom_point(aes(x_q2,y_q2), size = 4) +
    geom_point(aes(x_q3,y_q3), size = 4) +
    geom_point(aes(x_avg, y_avg), color = "blue", size = 4) +
    geom_text(aes(x_q1, y_q1, label = lab_q1), hjust = 0, nudge_x = nudge_x) +
    geom_text(aes(x_q2, y_q2, label = lab_q2), hjust = 0, nudge_x = nudge_x) +
    geom_text(aes(x_q3, y_q3, label = lab_q3), hjust = 0, nudge_x = nudge_x) +
    geom_text(aes(x_avg, y_avg, label = lab_avg), hjust = 0, nudge_y = -0.05) +
    scale_y_continuous(labels = function(x) scales::percent(x, digits)) +
...
plots[["engines"]] <-
  ...
  geom_col(fill = "lightgreen",color = "darkgreen") +
  geom_text(aes(label = scales::percent(ratio,.1)), vjust = 0, nudge_y = .02) +
...
plots[["engines2"]] <-
  ...
  ggplot(aes(fav_engine, ratio)) +
  geom_col(fill = "lightgreen",color = "darkgreen") +
  geom_text(aes(label = scales::percent(ratio,.1)), vjust = 0, nudge_y = .001) +
  ggtitle("Most commonly used browser among Google competitors") +
  scale_y_continuous(labels = function(x) scales::percent(x,1)) +
  labs(x = "", y = "% of users")

save_plot(plots[["engines"]], "engines")
save_plot(plots[["engines2"]], "engines2")
...
plots[["how_do_you_use_gs"]] <-
  ...
  geom_col(fill = "lightgreen",color = "darkgreen") +
  geom_text(aes(label = scales::percent(ratio,.1)), vjust = 0, nudge_y = .01) +
  ggtitle("Frequency of use of Google search") +
  scale_y_continuous(labels = function(x) scales::percent(x,1)) +
  labs(x = "", y = "% of users")

save_plot(plots[["how_do_you_use_gs"]], "how_do_you_use_gs")
```
...
plots[[item]] <- lollipop_chart(
  full_data, item,
  "Suggested searches are used 23% of the time", "% of users who selected a suggestion",
...
save_plot(plots[[item_facets]], item_facets)
...
plots[[item_facets]] <- quantile_chart2(
  full_data, item,
  "Time to complete a search by question",
  "time (seconds)", "% of users who complete the task before time",
  meanlab = "Average: %s sec", qlab = "%s%% complete before %s sec",
  xmax = 250, nudge_x = 8)
...
plots[[item2]] <- lollipop_chart(
  full_data, item, 
  "Depending on the query the search takes an average time of 45 sec to 1 min 30", "time (seconds)",
  fun = ~ mean(., na.rm =TRUE),
  percent = FALSE,
  nudge_x = 8)
